...
::
:: db/ write strategy:
::   Ingest writes to LOCAL_DB (%LOCALAPPDATA%\BurnRate\db) — no OneDrive/FUSE race.
...
set "JSONL_PREFIX=C--Users-WilcoDeTree-OneDrive---Valona-Intelligence-Claude-Cowork-Output"
...
:: Step 0a — ensure local db dir exists
if not exist "%LOCAL_DB%" mkdir "%LOCAL_DB%"

:: Step 0b — sync read-side files OneDrive -> local before ingest
...
:: Step 1 — mirror JSONL files
:: 1a: Claude Code CLI sessions — writes to %USERPROFILE%\.claude\projects\
...
:: 1b: Cowork (desktop app) sessions — Store app virtualises AppData\Roaming inside:
...
:: Steps 2-5 — all write to local db
...
:: Step 6 — Python copies local db -> OneDrive (handles paths with spaces reliably)
